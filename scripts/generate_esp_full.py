...
EXTERIOR_REFR_LIMIT = None  # Set to int to limit exterior REFRs for testing
...
# Magnus.esm cell formIDs for our exterior grids.
# We add REFRs to these cells via children GRUPs without overriding the CELL
# records themselves (which would crash CK).
EXISTING_CELL_FIDS = {
    (-2, -10): 0x010488FA,
    (-2, -9): 0x01047C5B,
}
...
def main():
    ...
    # Unique cells -> CELL formid (skip "Seyda Neen" since we use existing Magnus cells)
    cells = {}
    cell_order = []
    for row in placements:
        cell = row["cell"].strip()
    ...
    # REFRs per cell (include "Seyda Neen" for exterior refs)
    refrs_by_cell = {cell: [] for cell in cells}
    refrs_by_cell["Seyda Neen"] = []
    ...
    for row in placements:
        ...
        if cell == "Seyda Neen":
            # Exterior: compute Morrowind grid and cell-relative coordinates.
            ...
            # Interior: use absolute Morrowind coordinates directly (×50 for cm).
            grid_x = 0
            grid_y = 0
            x = x_mw * 50.0
            y = y_mw * 50.0
        # Z is already in cm in Morrowind data.
        z = z_mw
    ...
    # Exterior cells use existing Magnus formIDs, so they don't count as new records.
    exterior_grids = set()
    ...
    num_records = len(stat_records) + len(cells) - 1 + total_refr + 1 + 1  # -1 for unused Seyda Neen cell, +1 persistent, +1 WRLD
    ...
    # Use Magnus's persistent cell formID (0x0100E954) for the Morrowind WRLD.
    # Do NOT override the CELL record — just add the children GRUP.
    # Creating a new persistent cell causes "Cell will be destroyed" warning.
    PERSISTENT_CELL_FID = 0x0100E954
    persistent_grup = io.BytesIO()
    write_grup(persistent_grup, PERSISTENT_CELL_FID, 8, b"")
    persistent_children = io.BytesIO()
    write_grup(persistent_children, PERSISTENT_CELL_FID, 6, persistent_grup.getvalue())
    ...
    for block_label, subblocks in sorted(block_map.items(), key=lambda kv: kv[0]):
        ...
        for sub_label, grids in sorted(subblocks.items(), key=lambda kv: kv[0]):
            ...
            for grid_x, grid_y in grids:
                ...
                # Use Magnus's existing cell formID for these grids.
                cell_fid = EXISTING_CELL_FIDS[(grid_x, grid_y)]
                ...
                # Do NOT write a CELL record override — just the children GRUP with REFRs.
                # Overriding Magnus's CELL subrecords causes CK crashes.
