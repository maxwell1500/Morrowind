...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'SeydaNeen_ministat_10_onegrid.esp')
...
# Only first 10 unique converted objects as STATs
stat_fids = []
stat_records = []
for obj in sorted(converted_objects)[:10]:
    fid = alloc()
    stat_fids.append(fid)
    stat_records.append((fid, obj, ('morrowind\\%s.nif' % obj).encode('ascii')))

marker_fid = stat_fids[0]
...
for row in placements:
    ...
    if obj not in converted_objects:
        continue
...
# Exterior cell (-2,-10) with first 10 refs
grid_x, grid_y = -2, -10
grid_refs = [r for r in refrs if (r['gx'], r['gy']) == (grid_x, grid_y)][:10]
cell_fid = alloc()
cell_sub = io.BytesIO()
...
write_grup(exterior_block, 0, 5, subblock.getvalue())
