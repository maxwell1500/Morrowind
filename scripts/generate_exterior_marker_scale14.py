...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'SeydaNeen_exterior_marker_scale14.esp')
...
MW_CELL_SIZE = 8192.0
SCALE = 1.4  # Morrowind unit to cm
...
for row in placements:
    ...
    x = (x_mw - gx*MW_CELL_SIZE)*SCALE
    y = (y_mw - gy*MW_CELL_SIZE)*SCALE
    z = z_mw
    rx = float(row['rot_x']); ry = float(row['rot_y']); rz = float(row['rot_z'])
    refrs_by_cell[cell].append({
        'formid': alloc(), 'name': marker_fid,
        'x': x, 'y': y, 'z': z,
        'rx': rx, 'ry': ry, 'rz': rz,
        'grid_x': gx, 'grid_y': gy,
    })
...
write_subrecord(buf, 'CNAM', b'Seyda Neen Ext Marker Scale1.4\x00')
...
print('Created %s size=%d, exterior REFRs=%d' % (OUTPUT_FILE, buf.tell(), len(refrs_by_cell.get('Seyda Neen', []))))
print('Coordinate ranges:')
allx = [r['x'] for r in refrs_by_cell.get('Seyda Neen', [])]
ally = [r['y'] for r in refrs_by_cell.get('Seyda Neen', [])]
allz = [r['z'] for r in refrs_by_cell.get('Seyda Neen', [])]
print('  x: %.0f..%.0f' % (min(allx), max(allx)))
print('  y: %.0f..%.0f' % (min(ally), max(ally)))
print('  z: %.0f..%.0f' % (min(allz), max(allz)))
