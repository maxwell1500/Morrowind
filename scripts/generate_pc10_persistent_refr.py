...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "SeydaNeen_pc10_persistent_refr.esp")
...
num_records = len(stat_records) + 1 + 1 + 10 + 1 + 1
write_subrecord(buf, "HEDR", struct.pack("<fII", 0.96, num_records, 0xFE000000 | next_formid))
write_subrecord(buf, "CNAM", b"Persistent REFR Test\x00")
...
# Persistent cell with 10 REFRs (type 8)
...
# Empty grid cell (-2,-10) with no REFRs
grid_x, grid_y = -2, -10
...
print("10 REFRs in persistent cell (type 8), empty grid cell")
