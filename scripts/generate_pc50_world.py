...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "SeydaNeen_pc50_world.esp")
...
for row in placements:
    ...
    # World coordinates for persistent cell
    x = x_mw * 50.0
    y = y_mw * 50.0
    z = z_mw
...
grid_refs = [r for r in refrs if (r["gx"], r["gy"]) == (-2, -10)][:50]
...
num_records = len(stat_records) + 1 + 1 + 50 + 1 + 1
write_subrecord(buf, "HEDR", struct.pack("<fII", 0.96, num_records, 0xFE000000 | next_formid))
write_subrecord(buf, "CNAM", b"World Coord 50 REFR\x00")
...
# Persistent cell with 50 REFRs at WORLD coordinates
...
for refr in grid_refs:
    ...
    write_record(persistent_refr, "REFR", alloc(), 0x00010400, refr_sub)

persistent_grup = io.BytesIO()
write_grup(persistent_grup, persistent_fid, 8, persistent_refr.getvalue())
...
# Empty grid cell (-2,-10) using existing formID
grid_x, grid_y = -2, -10
...
write_subrecord(cell_sub, "XILS", struct.pack("<f", 1.0))
cell_buf = io.BytesIO()
write_compressed_record(cell_buf, "CELL", EXISTING_CELL_FID, 0, cell_sub)
cell_children = io.BytesIO()
write_grup(cell_children, EXISTING_CELL_FID, 6, b"")
...
print("50 REFRs in persistent cell with WORLD coordinates")
