...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'SeydaNeen_exterior_marker_10_onegrid.esp')
...
num_records = len(stat_records) + len(cells) + 1 + 10 + 1
write_subrecord(buf, 'HEDR', struct.pack('<fII', 0.96, num_records, 0xFE000000 | next_formid))
write_subrecord(buf, 'CNAM', b'Seyda Neen 10 One Grid\x00')
...
# One exterior cell (-2,-10) with first 10 refs
grid_x, grid_y = -2, -10
