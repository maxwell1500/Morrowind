        ...
        while inner < gend:
            if data[inner:inner+4] == b'WRLD':
                ...
                if data[cg_pos:cg_pos+4] == b'GRUP':
                    walk(data, cg_pos, found)
                break
            inner += 24 + struct.unpack('<I', data[inner+4:inner+8])[0]
        break
    pos += struct.unpack('<I', data[pos+4:pos+8])[0]

print(f'Found {len(found)} exterior cells with XCLC in Morrowind WRLD')
for x, y, fid in sorted(found, key=lambda t: (t[1], t[0]))[:80]:
    print(f'  grid ({x:4d}, {y:4d}) fid=0x{fid:08X}')
