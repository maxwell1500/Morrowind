...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'SeydaNeen_exterior_marker_onegrid.esp')
...
total_refr = 5
num_records = len(stat_records) + len(cells) + 1 + total_refr + 1
write_subrecord(buf, 'HEDR', struct.pack('<fII', 0.96, num_records, 0xFE000000 | next_formid))
write_subrecord(buf, 'CNAM', b'Seyda Neen One Grid 5\x00')
...
# One exterior cell (-2,-10) with 5 marker REFRs
grid_x, grid_y = -2, -10
grid_refs = [r for r in refrs_by_cell['Seyda Neen'] if (r['grid_x'], r['grid_y']) == (grid_x, grid_y)][:5]
cell_fid = alloc()
cell_sub = io.BytesIO()
