...
def main():
    ...
    for row in placements:
        ...
        # Starfield coordinates relative to the Morrowind cell origin.
    ...
    num_records = len(stat_records) + len(cells) + len(exterior_grid_cells) - 1 + total_refr + 1
    ...
    # Starfield WRLD children use block type 4 and sub-block type 5 (cell blocks).
    ...
    for grid_x, grid_y in exterior_grids:
        ...
        subblock.write(cell_buf.getvalue())
        subblock.write(cell_children.getvalue())
        # Cell sub-block inside a cell block; label is just an index for our small worldspace.
