"""Generate SeydaNeen.esp with all exterior REFRs but empty interior cells for CK crash isolation."""
import os
import struct
import io
import csv
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "SeydaNeen_exterior_only.esp")
...
def main():
    ...
    write_subrecord(buf, "CNAM", b"Seyda Neen Exterior Only\x00")
    ...
    # Exterior cells with all exterior REFRs
    ...
    print("Size: %d bytes" % buf.tell())
    print("STATs: %d, interior cells: %d, exterior grids: %d, exterior REFRs: %d" % (
        len(stat_records), len(cell_order) - 1, len(exterior_grids),
        len(refrs_by_cell.get("Seyda Neen", []))))
