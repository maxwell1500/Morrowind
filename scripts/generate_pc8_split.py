...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "SeydaNeen_pc8_split.esp")
...
num_records = len(stat_records) + 1 + 2 + 8 + 1 + 1
write_subrecord(buf, "HEDR", struct.pack("<fII", 0.96, num_records, 0xFE000000 | next_formid))
write_subrecord(buf, "CNAM", b"241STAT 8SPLIT 2CELL\x00")
...
# Two exterior cells: 4 REFRs each
for i, (grid_x, grid_y) in enumerate([(-2, -10), (-2, -9)]):
    chunk = grid_refs[i*4:(i+1)*4]
...
print("241 STATs + 8 REFRs split 4+4 across two cells")
