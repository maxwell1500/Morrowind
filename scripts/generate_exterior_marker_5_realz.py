...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'SeydaNeen_exterior_marker_5_realz.esp')
...
num_records = len(stat_records) + len(cells) + 1 + 5 + 1
write_subrecord(buf, 'HEDR', struct.pack('<fII', 0.96, num_records, 0xFE000000 | next_formid))
write_subrecord(buf, 'CNAM', b'Seyda Neen Real Z 5\x00')
...
# One exterior cell (-2,-10) with 5 marker REFRs using real z (not *50)
grid_x, grid_y = -2, -10
grid_refs = [r for r in refrs if (r['gx'], r['gy']) == (grid_x, grid_y)][:5]
cell_fid = alloc()
cell_sub = io.BytesIO()
...
print('First 5 real-z coords:')
for r in grid_refs:
    print('  (%.0f, %.0f, %.0f)' % (r['x'], r['y'], r['z']))
