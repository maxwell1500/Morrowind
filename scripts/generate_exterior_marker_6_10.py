...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'SeydaNeen_exterior_marker_6_10.esp')
...
write_subrecord(buf, 'CNAM', b'Seyda Neen Refs 6-10\x00')
...
# One exterior cell (-2,-10) with refs 6-10 (indices 5-9)
grid_x, grid_y = -2, -10
grid_refs = [r for r in refrs if (r['gx'], r['gy']) == (grid_x, grid_y)][5:10]
cell_fid = alloc()
cell_sub = io.BytesIO()
...
print('Refs 6-10 coords:')
for r in grid_refs:
    print('  (%.0f, %.0f, %.0f) rot=(%.1f,%.1f,%.1f)' % (r['x'], r['y'], r['z'], r['rx'], r['ry'], r['rz']))
