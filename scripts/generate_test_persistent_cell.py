...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "SeydaNeen_test_persistent_cell.esp")
...
stat_formids = {}
stat_records = []
for obj in sorted(converted_objects)[:10]:
    fid = alloc()
...
# Get exterior refs for grid (-2,-10)
refrs = []
for row in placements:
...
grid_refs = [r for r in refrs if (r["gx"], r["gy"]) == (-2, -10)][:10]
...
num_records = 10 + 3 + 10 + 1  # 10 STAT + 1 persistent CELL + 1 exterior CELL + 1 interior + 10 REFR + WRLD
write_subrecord(buf, "HEDR", struct.pack("<fII", 0.96, num_records, 0xFE000000 | next_formid))
write_subrecord(buf, "CNAM", b"Persistent Cell Test\x00")
...
# WRLD group with persistent cell + exterior cell
wrld_children = io.BytesIO()

# Persistent cell at grid (INT_MAX, INT_MAX)
...
# One dummy REFR in persistent cell (type 8)
refr_sub = io.BytesIO()
...
print("10 exterior REFRs + persistent cell at grid (INT_MAX, INT_MAX)")