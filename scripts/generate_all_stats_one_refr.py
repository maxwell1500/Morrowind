...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'SeydaNeen_all_stats_one_refr.esp')
...
num_records = len(stat_records) + 1 + 2 + 2 + 1  # STAT + dummy interior + 2 exterior + 2 refr + WRLD
write_subrecord(buf, 'HEDR', struct.pack('<fII', 0.96, num_records, 0xFE000000 | next_formid))
write_subrecord(buf, 'CNAM', b'All STATs One REFR\x00')
...
# Exterior cells: one REFR each, use first converted object as marker
first_obj = sorted(converted_objects)[0]
name_fid = stat_formids[first_obj]
exterior_block = io.BytesIO()
for idx, (grid_x, grid_y) in enumerate([(-2, -10), (-2, -9)]):
    cell_fid = alloc()
    cell_sub = io.BytesIO()
    ...
    refr_fid = alloc()
    refr_sub = io.BytesIO()
    write_subrecord(refr_sub, 'NAME', struct.pack('<I', name_fid))
    ...
    write_grup(exterior_block, idx, 5, subblock.getvalue())
...
print('Created %s size=%d with %d STATs' % (OUTPUT_FILE, buf.tell(), len(stat_records)))
